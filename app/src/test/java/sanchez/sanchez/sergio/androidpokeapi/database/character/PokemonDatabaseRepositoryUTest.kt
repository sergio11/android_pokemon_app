package sanchez.sanchez.sergio.androidpokeapi.database.character
...
import sanchez.sanchez.sergio.androidpokeapi.persistence.db.repository.exception.DBNoResultException
...
class PokemonDatabaseRepositoryUTest {
    ...
    fun test_database_repository_001_no_cached_characters() {
        runBlocking {
            try {
                withContext(IO) {
                    pokemonDBRepository.findCharactersOrderByNameAsc()
                }
            } catch (ex: Exception) {
    ...
    fun test_database_repository_002_characters_are_cached_successfully() {
        ...
        runBlocking {
            ...
            val charactersInDB = withContext(IO) {
                pokemonDBRepository.saveCharacters(characterList)
                pokemonDBRepository.findCharactersOrderByNameAsc()
            }
    ...
    fun setup() {
        ...
        pokemonDBRepository = PokemonDBRepositoryImpl(
            database.pokemonDAO(),
            PokemonDBMapper()
        )
